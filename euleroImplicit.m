...
ud = zeros(NDi,1);
for i=1:NDi
    ud(i) = gDi(t, pointDi(i,1), pointDi(i,2));
end
...
for e=1:nedgeBorders
    ...
    if ii > 0
        bNeumann(ii) = bNeumann(ii) + (gNe(t, Vb(1),Vb(2))/3 + gNe(t, Ve(1),Ve(2))/6)*edgeLen;
    end
    ii = geom.pivot.pivot(indexE);
    if ii > 0
        bNeumann(ii) = bNeumann(ii) + (gNe(t, Vb(1),Vb(2))/6 + gNe(t, Ve(1),Ve(2))/3)*edgeLen;
    end
end
F = fn + bNeumann + Ad*ud; % Termine noto da utilizzare
% Usiamo ora Eulero implicito
x = (B+dt*A)\(B*utemp+dt*F);
